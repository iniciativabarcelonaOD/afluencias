...
def insert_municipis(cursor):
    ...
    with open(file=filename, mode='r', encoding='UTF-8') as csvfile:
        ...
        for row in reader:
            ...
            values.append((nom, 1, cens, dia_mercat, renda_bruta_familiar_disponible, lloguer_mensual, index_envelliment))
...
def load_mercats(cursor):
    cursor.execute("SELECT id, nom, ST_X(posicio), ST_Y(posicio) FROM mercats;")    # ******************************* CHANGE THIS ********
...
def load_indiv_market_this_week(name, cnx):
    df = pd.read_sql(f"""
...
def load_indiv_market_last_week(name, cnx):
    df = pd.read_sql(f"""
        SELECT value, day_of_week
        FROM populartimes pt
            INNER JOIN mercats mer
...
def load_indiv_market_all_time(name, cnx):
    df = pd.read_sql(f"""
        SELECT value, day_of_week
        FROM populartimes pt
            INNER JOIN mercats mer
